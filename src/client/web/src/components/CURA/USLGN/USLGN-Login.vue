...
export default {
	...
	setup(props, { emit }) {
		...
		const fullName = computed(()=>store.state.globalStore.UserInfo.User_First_Name +" "+store.state.globalStore.UserInfo.User_Last_Name);
		...
		const userAuthentication = async () => {
			...
			if (res.Type === "RESPONSE") {
				let userInfo = {
					...
					User_First_Name: res.Response.User_First_Name,
					User_Last_Name: res.Response.User_Last_Name,
