      ...
      <div class="contentPage">
        <div class="d-flex flex-row my-4">
          ...
          <div class="col-9">
            <div class="d-flex flex-wrap">
              <div
                class="col-12 col-sm-6 form-floating m-3"
        ...
        <div class="d-flex flex-row">
          ...
          <div class="col-9">
            <div class="d-flex flex-column pb-3">
              <div
                v-for="(day, index) in Global_Definition.Working_Days"
                :key="index"
                class="col shadow-sm p-2 m-3 bg-body rounded"
              >
                <working-day
                  :number="index"
                  :workingDay="day"
                  :numOfDays="Global_Definition.Working_Days.length"
                  :days="days"
                  @remove-working-day="removeWorkingDay"
                />
              </div>
              <button
                v-if="Global_Definition.Working_Days.length < 7"
                @click="addWorkingDay"
                class="col-6 col-md-4 col-xll-2 btn btn-info m-3"
              >
                + Add Day
              </button>
            </div>
            ...
            <div class="d-flex flex-wrap">
              ...
              >
                ...
                <datalist id="mandalOptions">
                  <option
                    v-for="mandal in mandalList"
                    :key="mandal.index"
                    :value="mandal.listName"
                  ></option>
                </datalist>
        ...
        <div class="d-flex flex-row justify-content-end">
          ...
          <button
            v-show="true"
            @click="changePage"
            class="col-2 btn btn-secondary m-3"
          >
            Next
          </button>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const dateSelected = async (event) => {
      ...
      Global_Definition.value.Institute_Foundation_Date = new Date(
        instituteInfo.value.foundationDate
      ).getTime();
    ...
    const addWorkingDay = async () => {
      if (Global_Definition.value.Working_Days.length < 7) {
        Global_Definition.value.Working_Days.push(
          JSON.parse(JSON.stringify(workingDayInfo.value))
        );
      }
    };
    ...
    const fetchCurrency = async (event) => {
      try {
        ...
        if (
          ...
          instituteInfo.value.selectedCurrency = currencyList.value.find(
            (currency) => {
              return selectedCurrency === currency.listName;
            }
          );
          Global_Definition.value.Default_Currency =
            instituteInfo.value.selectedCurrency.listCode;
    ...
    const fetchTimezone = async (event) => {
      try {
        ...
        if (
          ...
          // determine if the value is in the datalist. If so, someone selected a value in the list!
          instituteInfo.value.selectedTimezone = timezoneList.value.find(
            (timezone) => {
              return selectedTimezone === timezone.listName;
            }
          );
          Global_Definition.value.Default_Time_Zone =
            instituteInfo.value.selectedTimezone.listCode;
    ...
    const fetchCountry = async (event) => {
      try {
        ...
        if (
          ...
          // determine if the value is in the datalist. If so, someone selected a value in the list!
          instituteInfo.value.selectedCountry = countryList.value.find(
            (country) => {
              return selectedCountry === country.listName;
            }
          );
          Global_Definition.value.Institute_Address.Country =
            instituteInfo.value.selectedCountry.listCode;
    ...
    const fetchState = async (event) => {
      try {
        ...
        if (
          ...
          Global_Definition.value.Institute_Address.State =
            instituteInfo.value.selectedState.listCode;
          validateInput("State");
          await fetchDistrict();
        } else {
    ...
    const fetchDistrict = async (event) => {
      try {
        ...
        if (
          ...
          // determine if the value is in the datalist. If so, someone selected a value in the list!
          instituteInfo.value.selectedDistrict = districtList.value.find(
            (district) => {
              return selectedDistrict === district.listName;
            }
          );
          Global_Definition.value.Institute_Address.District =
            instituteInfo.value.selectedDistrict.listCode;
    ...
    const fetchMandal = async (event) => {
      try {
        ...
        if (
          ...
          // determine if the value is in the datalist. If so, someone selected a value in the list!
          instituteInfo.value.selectedMandal = mandalList.value.find(
            (mandal) => {
              return selectedMandal === mandal.listName;
            }
          );
          Global_Definition.value.Institute_Address.Mandal =
            instituteInfo.value.selectedMandal.listCode;
    ...
    const fetchPostalCode = async () => {
      try {
        ...
        let res = await store.dispatch("sendHTTPReq", req);
        if (res.Type === "RESPONSE") {
          Global_Definition.value.Institute_Address.Postal_Code =
            res.Response.Postal_Code;
    ...
    const validateInput = async (inputType) => {
      ...
      if (
        ...
        Global_Definition.value.Institute_Address.Street_Address != "" &&
        Global_Definition.value.Institute_Address.Postal_Code != null &&
        Global_Definition.value.Institute_Address.Postal_Code != "" &&
    ...
    const checkGlobalDefinitionInput = () => {
      ...
      for (const key in Global_Definition.value) {
        if (
          Global_Definition.value[key] === null ||
          Global_Definition.value[key] === ""
        ) {
          result = false;
        }
      }
    ...
    const clearForm = async () => {
      ...
      Global_Definition.value.Working_Days = [
        { Day: null, Hour_Open: null, Hour_Close: null },
      ];
